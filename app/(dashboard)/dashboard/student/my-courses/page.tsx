        ...
        if (coursesData) {
          const formattedCourses = coursesData.map((course) => {
            ...
            const courseLectures =
              lecturesData?.filter((l) => l.course_id === course.id) || [];
            const courseAttendance =
              attendanceData?.filter((a) => a.course_id === course.id) || [];
            const attendedLectures = courseAttendance.filter(
              (a) => a.status === "present"
            ).length;
                        ...
                        <div className="flex items-center text-sm bg-blue-50 p-2 rounded-md">
                          ...
                          <div>
                            <p className="font-medium">
                              Next Lecture: {course.nextLecture.title}
                            </p>
                            <p className="text-xs text-gray-500">
                              {format(
                                new Date(course.nextLecture.date),
                                "PPP p"
                              )}
                            </p>
                          </div>
