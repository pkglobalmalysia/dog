...
export default function AdminDashboardPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Calculate stats from loaded data using local variables
      setStats({
        total_students: studentsData.length,
        total_teachers: teachersData.length,
        pending_payments: paymentsData.filter(
          (p: any) => p.payment.status === "pending"
        ).length,
        total_revenue: paymentsData
          .filter((p: any) => p.payment.status === "approved")
          .reduce((sum: number, p: any) => sum + p.payment.amount, 0),
      });
    } catch (error) {
  ...
  const loadStudentPayments = async (student: Student) => {
    try {
      ...
      const response = await fetch(
        `/api/admin/student-payments/${student.id}`,
        {
          credentials: "include",
        }
      );
  ...
  const loadTeacherSalaries = async (teacher: Teacher) => {
    try {
      ...
      const response = await fetch(
        `/api/admin/teacher-salaries/${teacher.id}`,
        {
          credentials: "include",
        }
      );
  ...
  const loadStudentEnrollments = async (student: Student) => {
    try {
      const response = await fetch(
        `/api/admin/student-enrollments/${student.id}`,
        {
          credentials: "include",
        }
      );
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/update-student/${selectedStudent.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  const handleDeleteEnrollment = async (enrollmentId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/delete-enrollment/${enrollmentId}`,
        {
          method: "DELETE",
          credentials: "include",
        }
  ...
  const handleAddManualSalary = async () => {
    if (
      !selectedTeacher ||
      !manualSalaryForm.total_amount ||
      !manualSalaryForm.total_classes
    ) {
      toast({
        title: "Error",
  ...
  const handleUpdateSalaryStatus = async (salaryId: string, status: string) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/update-salary-status/${salaryId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
  ...
  return (
    <div className="container mx-auto py-4 sm:py-6 lg:py-8 px-2 sm:px-4">
      <div className="mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold mb-2">Admin Management Dashboard</h1>
      ...
      >
        <TabsList className="grid w-full grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 h-auto">
          ...
          <TabsTrigger value="create-user" className="text-xs sm:text-sm px-2 py-2 col-span-2 sm:col-span-1">Create User</TabsTrigger>
        </TabsList>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-xl sm:text-2xl font-bold">
                  {loading
                    ? "..."
                    : `RM${stats.total_revenue?.toFixed(2) || "0.00"}`}
                </div>
              </CardContent>
            </Card>
          </div>
              ...
              <div className="block sm:hidden space-y-3">
                {loading ? (
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    {students.length === 0
                      ? "No students found. Students will appear here when they register."
                    ...
                    <Card key={student.id} className="p-4">
                      <div className="space-y-2">
                        ...
                        <div className="text-xs">
                          Enrollments: {student.active_enrollments}/{student.enrollments_count}
                        </div>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleDeleteUser(student.id, student.full_name)
                                }
                              >
        ...
        <TabsContent value="teachers" className="space-y-4 sm:space-y-6">
          <Card>
            <CardHeader>
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                <div>
                  <CardTitle className="text-lg sm:text-xl">Teachers ({filteredTeachers.length})</CardTitle>
                  <CardDescription className="text-sm">
            ...
            <CardContent>
              ...
              <div className="block sm:hidden space-y-3">
                {loading ? (
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    {teachers.length === 0
                      ? "No teachers found. Teachers will appear here when they register."
                  ...
                  filteredTeachers.map((teacher) => (
                    <Card key={teacher.id} className="p-4">
                      <div className="space-y-2">
                        <div className="flex justify-between items-start">
                          <div>
                            <h3 className="font-medium text-sm">{teacher.full_name}</h3>
            ...
            <CardContent>
              ...
              <div className="block sm:hidden space-y-3">
                {loading ? (
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    No payments found. Payments will appear here when
                    students submit payment receipts.
                  </div>
                ) : (
                  payments.map((payment) => (
                    <Card key={payment.id} className="p-4">
                      <div className="space-y-2">
                        <div className="flex justify-between items-start">
                          <div>
                            <h3 className="font-medium text-sm">{payment.student?.name || 'Unknown Student'}</h3>
                        ...
                        <div className="text-sm font-medium">
                          RM{payment.payment.amount.toFixed(2)}
                        </div>
                        <div className="text-xs text-muted-foreground">
                          Method: {payment.payment.payment_method}
                        </div>
                        <div className="text-xs text-muted-foreground">
                          Date: {new Date(payment.payment.created_at).toLocaleDateString()}
                        </div>
                        <div className="flex items-center gap-2 pt-2">
                    ...
                    {loading ? (
                      ...
                      payments.map((payment) => (
                        <TableRow key={payment.id}>
                          ...
                          <TableCell>{payment.course?.title || 'Unknown Course'}</TableCell>
                          <TableCell>
                            RM{payment.payment.amount.toFixed(2)}
                          </TableCell>
        ...
        <TabsContent value="create-user" className="space-y-4 sm:space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg sm:text-xl">Create New User</CardTitle>
              <CardDescription className="text-sm">
                Create a new student or teacher account with Supabase
                authentication
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 sm:space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="space-y-4">
                  <h3 className="text-base sm:text-lg font-semibold">Create Student</h3>
                  <div className="space-y-4">
                    <div>
                ...
                <div className="space-y-4">
                  <h3 className="text-base sm:text-lg font-semibold">Create Teacher</h3>
                  <div className="space-y-4">
                    <div>
                      <label className="text-sm font-medium">Email</label>
                      <Input
                    ...
                    <Button
                      ...
                      disabled={
                        ...
                        !newTeacherForm.full_name ||
                        !newTeacherForm.ic_number
                      }
                      className="w-full"
                    >
            ...
            <form onSubmit={handleUpdateUser} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditForm({ ...editForm, ic_number: e.target.value })
                  }
                  className="w-full border rounded px-3 py-2 text-sm"
                />
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                <button
                  ...
                  className="flex-1 bg-gray-300 text-gray-700 py-2 px-4 rounded hover:bg-gray-400 text-sm font-medium"
                >
                  Cancel
                </button>
              </div>
      ...
      {showStudentDetails && selectedStudent && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="bg-white rounded-lg p-3 sm:p-4 lg:p-6 max-w-6xl w-full max-h-[95vh] overflow-y-auto">
                  ...
                  {editingProfile ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <Badge
                          variant={
                            selectedStudent.approved ? "default" : "secondary"
                          }
                        >
                        ...
                        >
                          ...
                          {availableCourses.map((course) => (
                            <option
                              key={course.id}
                              value={course.id}
                              title={`${course.title} - RM ${course.price}`}
                            >
                          ...
                          studentEnrollments.map((enrollment) => (
                            ...
                            >
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-sm text-gray-600">
                                  Price: RM{" "}
                                  {enrollment.courses_enhanced?.price || "N/A"}
                                </p>
                                {enrollment.admin_notes && (
                                  <p
                                    className="text-xs text-gray-500 truncate"
                                    title={enrollment.admin_notes}
                                  >
                ...
                <CardHeader>
                  <CardTitle className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        setEditingTeacherProfile(!editingTeacherProfile)
                      }
                  ...
                  {editingTeacherProfile ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <div>
                        ...
                        {selectedTeacher.ic_number || "Not provided"}
                      </div>
                      <div>
                        <strong>Phone:</strong>{" "}
                      ...
                      <div>
                        ...
                        {selectedTeacher.specializations || "Not provided"}
                      </div>
                      <div>
                        <strong>Experience:</strong>{" "}
                        {selectedTeacher.experience_years || 0} years
                      </div>
                      <div>
                        <strong>Courses:</strong>{" "}
                        {selectedTeacher.courses_count || 0}
                      </div>
                      <div className="flex items-center gap-2">
                        <strong>Status:</strong>
                        <Badge
                          variant={
                            selectedTeacher.approved ? "default" : "secondary"
                          }
                        >
              ...
              <Card className="xl:col-span-2">
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div className="border rounded-lg p-4 bg-blue-50">
                      ...
                      <div className="flex gap-2 mt-3">
                        ...
                        <p className="text-sm text-gray-600 flex items-center">
                          Total: RM{" "}
                          {(
                            (parseFloat(manualSalaryForm.total_amount) || 0) +
                            (parseFloat(manualSalaryForm.bonus_amount) || 0)
                          ).toFixed(2)}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="bg-green-50 p-4 rounded-lg">
                        ...
                        <p className="text-2xl font-bold text-green-700">
                          ...
                          {teacherSalaries
                            .reduce(
                              (sum, salary) => sum + salary.final_amount,
                              0
                            )
                            .toFixed(2)}
                        </p>
                      ...
                      <div className="bg-orange-50 p-4 rounded-lg">
                        ...
                        <p className="text-2xl font-bold text-orange-700">
                          {teacherSalaries.reduce(
                            (sum, salary) => sum + salary.total_classes,
                            0
                          )}
                        </p>
                      </div>
                    ...
                    <div>
                      <h4 className="font-medium mb-2">
                        Salary History ({teacherSalaries.length})
                      </h4>
                          ...
                          teacherSalaries.map((salary) => (
                            ...
                            >
                              <div className="flex-1">
                                ...
                                <p className="text-sm text-gray-600">
                                  {salary.total_classes} classes • Base: RM{" "}
                                  {salary.total_amount.toFixed(2)}
                                  {salary.bonus_amount > 0 &&
                                    ` • Bonus: RM ${salary.bonus_amount.toFixed(
                                      2
                                    )}`}
                                </p>
                                <p className="text-xs text-gray-500">
                              ...
                              <div className="flex items-center gap-2">
                                <div className="text-right mr-2">
                                  <p className="font-bold text-lg">
                                    RM {salary.final_amount.toFixed(2)}
                                  </p>
                                </div>
                                <Badge
                                  variant={
                    ...
                    <div className="flex gap-2 pt-4 border-t">
                      <Button
                        onClick={() =>
                          window.open("/admin/salary-management", "_blank")
                        }
